...
function initVisitor() {
  try {
    const n = (parseInt(localStorage.getItem('tr7_vc') || '0', 10)) + 1;
    localStorage.setItem('tr7_vc', String(n));
  ...
  function tick(now) {
    ...
    if (ni !== msgIdx) {
      ...
      if (statEl) {
        ...
        setTimeout(() => { if (statEl) { statEl.textContent = LOADER_MSGS[msgIdx]; statEl.style.opacity = '1'; } }, 200);
      }
    }
...
const Galaxy = (() => {
  ...
  const COUNT = (() => {
    const a = W * H;
    if (a < 250000) return 4500;
    if (a < 600000) return 7500;
    return 11000;
  })();
  ...
  function buildGeo() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      let cr, cg, cb;
      if (t < 0.22) {
        /* Near core — warm white */
        cr = 1.0; cg = 0.8; cb = 0.76;
      } else if (t < 0.55) {
        const f = (t - 0.22) / 0.33;
        cr = 1.0;
        cg = 0.8 * (1 - f) + 0.0 * f;
...
function initGSAP() {
  ...
  gsap.fromTo('#main-card',
    { opacity: 0, y: 55 },
    { opacity: 1, y: 0, duration: 1.5, ease: 'expo.out', delay: 0.1 }
  );
  ...
  qsa('.app-card').forEach((card, i) => {
    ...
    gsap.fromTo(card,
      ...
      { opacity: 1, y: 0, scale: 1, duration: 0.72, ease: 'back.out(1.6)', delay: 0.55 + i * 0.07 }
    );
  ...
  gsap.fromTo('.site-footer',
    { opacity: 0, y: 18 },
    { opacity: 1, y: 0, duration: 1.1, ease: 'expo.out', delay: 1 }
  );
}
...
function initModal() {
  ...
  if (bannerWrap && bannerImg) {
    ...
    bannerWrap.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); t(); } });
  }
  if (profileWrap && profileImg) {
    const t = () => open(profileImg.src);
    profileWrap.addEventListener('click', t);
    profileWrap.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); t(); } });
  }
